...
function ensureFieldId(value) {
    if (FIELD_IDS.includes(value)) {
        return value;
    }
    throw new Error(`Unsupported field_id: ${value}`);
}
function topicToNodeData(topic) {
    return {
...
export async function listTopics(fieldId) {
...
export async function getSectionsByTopic(topicId) {
    const supabase = getSupabase();
...
export async function upsertGraphEdge(source, target, label) {
    if (source === target) {
...
export async function deleteGraphEdge(source, target, label) {
    const supabase = getSupabase();
