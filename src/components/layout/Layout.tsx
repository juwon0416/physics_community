...
export function Layout() {
    ...
    return (
        <div
            className={
                isHome || isGraph
                    ? 'relative flex h-screen flex-col overflow-hidden bg-background font-sans text-foreground'
                    : 'relative flex min-h-screen flex-col bg-background font-sans text-foreground'
            }
        >
            <Navbar />
            <main className={isHome || isGraph ? 'flex-1 overflow-hidden' : 'flex-1'}>
                <Outlet />
            </main>
            {!isHome && !isGraph && (
